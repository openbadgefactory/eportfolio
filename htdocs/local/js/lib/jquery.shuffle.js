...
(function (factory) {
  if (typeof define === 'function' && define.amd) {
    define(['jquery-loader', 'modernizr'], factory);
  } else {
...
Shuffle.prototype = {
  ...
  shuffle : function( category, sortObj ) {
    var self = this;
    // <EKAMPUS
    var skip_animation = arguments[2] || false;
    // EKAMPUS>
  ...
  // <EKAMPUS
  paginate : function ( sortOpts, opts, exclude_selector ) {
    var self = this;
//    var interval = null;
//    var delay = 50; // How often to check the layoutList size.
//    var timeout = 3000; // Timeout in ms

    // HACK: If we're in the middle of (previous) shuffling, the layout
    // functions (mainly _shrink and _reLayout) get confused and hide some
    // of the elements that should be shown because of race condition issues.
    // So that's why we check periodically, whether the layoutList is cleared
    // and we can go on with our next layout change.
    this.waitLayout(function () {
        var from = opts.page * opts.limit;
        var to = from + opts.limit;
        var excluded =  self.$items.filter(exclude_selector);
        var items = $(self.$items.filter('.filtered').sorted(sortOpts));
        var filtered = $.merge(excluded, items.not(exclude_selector).slice(from, to));
