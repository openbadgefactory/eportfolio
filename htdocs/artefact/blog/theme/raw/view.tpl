{include file="header.tpl"}
		    ...
		    {if $blogviewid}
			<a class="btn" href="{$WWWROOT}view/view.php?id={$blogviewid}">{str section="artefact.blog" tag="viewpublishedblog"}</a>
                        <a class="btn" href="{$WWWROOT}view/access.php?id={$blogviewid}">{str section="artefact.blog" tag="changeblogpageaccess"}</a>
		    {else}
		    {$createblogform|safe}
		    {/if}
                {if !$blog->get('locked')}
                    <a class="btn settings" href="{$WWWROOT}artefact/blog/settings/index.php?id={$blog->get('id')}">{str section="artefact.blog" tag="blogsettings"}</a>
                {/if}
                {* EKAMPUS> *}
				</div>
