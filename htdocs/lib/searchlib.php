 ...
 * @param string
 * @param string
 * @param boolean Whether to include the system groups in results
 * @param boolean Whether to include ONLY the system groups in results
 * @return array  A data structure containing results looking like ...
 *         $results = array(
 *               count   => integer, // total number of results
 *               limit   => integer, // how many results are returned
...
function search_group($query_string, $limit, $offset = 0, $type = 'member',
        $groupcategory = '', $includesystemgroups = true, $onlysystemgroups = false) {
// EKAMPUS>
    $plugin = get_config('searchplugin');
    safe_require('search', $plugin);

    // <EKAMPUS
    return call_static_method(generate_class_name('search', $plugin), 'search_group',
            $query_string, $limit, $offset, $type, $groupcategory,
            $includesystemgroups, $onlysystemgroups);
    // EKAMPUS>
}
