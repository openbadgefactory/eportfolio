...
function pieform_element_filebrowser_configure_tabs($viewowner, $prefix) {
    ...
    if ($viewowner['type'] == 'institution') {
        ...
        $tabs['user'] = get_string('myfiles', 'artefact.file');
        //< EKAMPUS
        $getsystemgroups = false;
        if ($groups = group_get_user_groups($USER->get('id'), null, $getsystemgroups)) {
        //if ($groups = $USER->get('grouproles')) {
            $tabs['group'] = get_string('groupfiles', 'artefact.file');
            require_once(get_config('libroot') . 'group.php');


            //$groups = group_get_user_groups($USER->get('id'), null, $getsystemgroups);
            // EKAmPUS >
...
function pieform_element_filebrowser_upload(Pieform $form, $element, $data) {
    ...
    try {
        ...
        // <EKAMPUS
        if ($parentfolder) {
            $cloud = cloud_provider($parentfolder);
            if ($cloud) {
                $cloud->upload($newid);
            }
        }
...
function pieform_element_filebrowser_createfolder(Pieform $form, $element, $data) {
    ...
    // <EKAMPUS
    if ($parentfolder && $parentartefact) {
        $cloud = cloud_provider($parentartefact);
        if ($cloud) {
            try {
                $cloud->create_folder($f);
            }
...
function pieform_element_filebrowser_update(Pieform $form, $element, $data) {
    ...
    // <EKAMPUS
    $cloud = cloud_provider($artefact);
    if ($cloud) {
        $cloud->update($artefact);
    }
    // EKAMPUS>
...
function pieform_element_filebrowser_move(Pieform $form, $element, $data) {
    ...
    $ok = false;
    if (!$cloud) {
        $cloud = cloud_provider($newparentid);
    }
    if ($cloud) {
        if (!$newparentid) {
            return array('error' => true, 'message' => get_string('illegalmove', 'artefact.cloudfile'));
        }
        $ok = $cloud->move($artefact, $newparentid);
    }
    else {
        $ok = $artefact->move($newparentid);
    }
...
function pieform_element_filebrowser_changefolder(Pieform $form, $element, $folder) {
    ...
    // Sync to cloud
    $cloud = get_field_sql(
        "SELECT cf.type FROM {artefact_cloudfile} cf
        INNER JOIN {artefact} a ON cf.artefact = a.id
        WHERE a.id = ? AND a.artefacttype = 'folder' AND a.parent IS NULL", array($folder)
    );
    if ($cloud) {
        $cf = new CloudFile($cloud);
        try {
            $cf->sync_artefacts();
        }
        catch (RemoteServerException $e) {
...
function pieform_element_filebrowser_get_headdata($element) {
    ...
    if ($element['config']['upload']) {
        ...
        // <EKAMPUS
        // Use our own non-minified dropzone for now to prevent issues with
        // requirejs.
        $headdata[] = '<script type="text/javascript" src="' . get_config('wwwroot') . 'js/dropzone/dropzone.js"></script>';
        // EKAMPUS>
