    ...
    public static function render_instance(BlockInstance $instance, $editing=false) {
        ...
        if (isset($configdata['select']) && $configdata['select'] == 2) {
          ...
          http://www.roytanck.com/get-my-flickr-widget/
                ...
                case 'panoramio':
                    // <EKAMPUS
                    // Panoramio slideshow doesn't work over HTTPS, so let's
                    // just use the built-in slideshow viewer.
                        if ($style == 1) {
                            $template = 'slideshow';
                            $height = round($width * 0.75);
                        }
                        ...
                        foreach ($data['photos'] as $photo) {
                            ...
                            $images[] = array(
                                'link' => $link,
                                'source' => ($style == 1 ? $link : $thumb), // EKAMPUS, show full image in slideshow
                                'title' => $title, // EKAMPUS, fix titles
                                'slimbox2' => $slimbox2attr
                            );
                        }
                    break;
                case 'photobucket':
                    // <EKAMPUS
                    // Photobucket slideshow doesn't work over HTTPS, so let's
                    // just use the built-in slideshow viewer.
                        if ($style == 1) {
                            $height = round($width * 0.75);
                            $template = 'slideshow';
                        ...
                        foreach ($photos as $photo) {
                            ...
                            $images[] = array(
                                'link' => $link,
                                'source' => ($style == 1 ? $link : $thumb), // EKAMPUS, use full image in slideshow
                                'title' => $description,
                                'slimbox2' => $slimbox2attr
                            );
                        }
...
//                'description' => get_string('widthdescription', 'blocktype.file/gallery'),
    ...
    private static function make_gallery_url($url) {
        static $embedsources = array(
            ...
            // Example: http://s9.photobucket.com/user/athenad/library/Flowers or
            // http://s65.photobucket.com/user/Ignwar/library/Album%20Fall%20Colors
            array(
                'match' => '#.*([a-zA-Z]+[0-9]+).photobucket.com/user/([a-zA-Z0-9\-\_]+)/library/([a-zA-Z0-9\.\,\:\;\@\-\_\+\%\ ]*).*#',
                'url'   => 'http://$1.photobucket.com/album/$2/$3',
                'type'  => 'photobucket',
                'var1' => '$2',
                'var2' => '$3',
            ),
            // Photobucket User Photos (direct link)
            // Example: http://s9.photobucket.com/user/athenad/library/
            array(
                'match' => '#.*([a-zA-Z]+[0-9]+).photobucket.com/user/([a-zA-Z0-9\-\_]+)/library.*#',
                'url' => 'http://$1.photobucket.com/album/$2',
                'type'  => 'photobucket',
                'var1' => '$2',
                'var2' => null,
            ),
