...
/// Check table doesn't exist
    ...
    // <EKAMPUS
    // Function table_exists() doesn't find temporary tables, so let's drop it first
    // before trying to recreate it. Another option would be
    // CREATE TEMPORARY TABLE tbl IF NOT EXISTS and then TRUNCATE tbl.
    array_unshift($sqlarr, 'DROP TEMPORARY TABLE IF EXISTS {' . $table->getName() . '}');
    // EKAMPUS>
