...
function general_account_prefs_form_elements($prefs) {
    ...
    */
    ...
    $elements['quotausage'] = array(
        'type'  => 'html',
        'title' => get_string('quota', 'mahara'),
    ...
    foreach ($institutions as &$userinst) {
        foreach ($userinst as $key => &$value) {
            ...
            foreach ($value as $k => $v) {
                ...
                // <EKAMPUS
                // Hide the links to prevent students from seeing staff lists.
//                $links[] = get_string('institutionlink', 'mahara', $url, $v);
                $links[] = '<span class="institution">' . $v . '</span>';
                // EKAMPUS>
            }
            switch ($key) {
                case 'admin':
