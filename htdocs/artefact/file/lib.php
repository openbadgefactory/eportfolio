...
abstract class ArtefactTypeFileBase extends ArtefactType {
    ...
    public static function get_my_files_data($parentfolderid, $userid, $group=null, $institution=null, $filters=null) {
        ...
        if ($institution) {
            ...
            // <EKAMPUS
            // Show institution files in filebrowser only if the user is
            // institution admin.
            //
            // See:
            // - https://gitlab.com/mahara/mahara/commit/3ba72d71f6243ff9cba51d9c3b1888d5ed56df55
            // - https://bugs.launchpad.net/mahara/+bug/1298646
            else {
                  $from .= '
    ...
    public static function files_form($page='', $group=null, $institution=null, $folder=null, $highlight=null, $edit=null) {
        ...
        if ( ! $group && ! $institution) {
            $form['elements']['quota'] = array(
                'type' => 'html',
                'value' => '<p>' . get_string('quotausage', 'mahara', sprintf('%0.1fMB', $USER->get('quotaused') / 1048576), sprintf('%0.1fMB', $USER->get('quota') / 1048567)) . '</p>'
            );
        }
        // EKAMPUS>
...
class ArtefactTypeFile extends ArtefactTypeFileBase {
    ...
    public static function get_icon($options=null) {
        global $THEME;
        // <EKAMPUS
        if (use_cloud_icon($options['id'])) {
            return $THEME->get_url('images/cloud_doc.png');
        }
        // EKAMPUS>
...
class ArtefactTypeFolder extends ArtefactTypeFileBase {
    ...
    public static function get_icon($options=null) {
        ...
        // <EKAMPUS
        if (use_cloud_icon($options['id'])) {
            return $THEME->get_url('images/cloudy.png');
        }
        // EKAMPUS>
