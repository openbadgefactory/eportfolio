...
class AuthLdap extends Auth {
        ...
        if ($ldapconnection) {
            ...
            if ($ldap_login) {
                if ($user->id && $update) {
                    ...
                    //<EKAMPUS
                    if ( ! empty($this->config['rolefield'])) {
                        $ldapattributes['role'] = $this->config['rolefield'];
                    }
                    //EKAMPUS>
                    ...
                    foreach ($fieldstoimport as $field) {
                        ...
                        //<EKAMPUS
                        // Set institution staff/admin roles
                        if ($field == 'role') {
                            $this->set_user_role($user->id, $ldapdetails);
                            continue;
                        }
                        //EKAMPUS>
        ...
        foreach ($ldap_contexts as $context) {
            ...
            if ($this->config['search_sub'] == 'yes') {
                ...
                $ldap_result = ldap_search($ldapconnection, $context, '(&(' . $this->config['user_attribute']
                    . '=' . $this->filter_addslashes($username) . ')(eduhelswlicense=eKampus))', array($this->config['user_attribute'])); // <EKAMPUS
    ...
    public function get_user_info($username) {
        ...
        //<EKAMPUS
        if ( ! empty($this->config['rolefield'])) {
            $attributes['role'] = $this->config['rolefield'];
        }
        //EKAMPUS>
    ...
    public function get_attribute_distinct_values($searchsub) {
        ...
        $filter = '(&' . $filter . '(' . $this->config['syncgroupsuserattribute'] . '=*)('.$extrafilterattribute.')(eduhelswlicense=eKampus))'; // <EKAMPUS
    ...
    public function sync_users($dryrun = false) {
        ...
        //<EKAMPUS
        if ( ! empty($this->config['rolefield'])) {
            $temptable->addFieldInfo('role', XMLDB_TYPE_TEXT);
        }
        //EKAMPUS>
            ...
            if (!$doupdate) {
                ...
                while ($record = $rs->FetchRow()) {
                    ...
                    if (!$dryrun) {
                        ...
                        //<EKAMPUS
                        $this->set_user_role($record['id'], $record);
                        //EKAMPUS>
                    }
            ...
            if (!$docreate) {
                ...
                while ($record = $rs->FetchRow()) {
                    ...
                    if (
                        ...
                        if (!$dryrun) {
                            $user_id = create_user($todb, array(), $this->institution);
                            //<EKAMPUS
                            $this->set_user_role($user_id, $record);
                            //EKAMPUS>
                        }
                        $nbcreated++;
                    }
    ...
    function sync_groups($dryrun = false) {
        ...
        $syncbyattribute = $this->get_config('syncgroupsbyuserfield') && $this->get_config('syncgroupsuserattribute'); //<EKAMPUS
        ...
        foreach ($groups as $group) {
            ...
            if (!$dbgroup = get_record('group', 'shortname', $shortname, 'institution', $this->institution)) {
                ...
                try {
                    ...
                    // <EKAMPUS
                    $institutionname = get_field('institution', 'displayname', 'name', $this->institution);
                    $basename = $institutionname . ' : ' . $group;
                    // EKAMPUS>
            ...
            delete_records('group_member', 'group', $groupid, 'member', '1');
            // EKAMPUS>
            //break;
        }
    ...
    //<EKAMPUS
    // Update user admin/staff roles in current institution.
    // User account must exist and already be institution member.
    public function set_user_role($user_id, $ldapdetails) {

        $ldapdetails = (array)$ldapdetails;

        $role = null;
        if (isset($ldapdetails['role'])) {
            $role = $ldapdetails['role'];
        }
        if (isset($ldapdetails['ldaprole'])) {
            $role = $ldapdetails['ldaprole'];
        }

        if ( !$role || !$user_id) {
            log_info('role or user id missing');
            return;
        }

        $admin_roles = array();
        if ( ! empty($this->config['adminrolevalue'])) {
            $admin_roles = array_map( function ($s) {return trim($s);}, explode(',', $this->config['adminrolevalue']) );
        }

        $staff_roles = array();
        if ( ! empty($this->config['staffrolevalue'])) {
            $staff_roles = array_map( function ($s) {return trim($s);}, explode(',', $this->config['staffrolevalue']) );
        }
        $whereobject = array(
            'usr' => $user_id,
            'institution' => $this->institution
        );
        $dataobject = $whereobject;
        if ( ! empty($admin_roles)) {
            $dataobject['admin'] = (int) in_array($role, $admin_roles);
        }
        if ( ! empty($staff_roles)) {
            $dataobject['staff'] = (int) in_array($role, $staff_roles);
        }

        update_record('usr_institution', $dataobject, $whereobject);
    }
    //EKAMPUS>
}
...
class PluginAuthLdap extends PluginAuth {
    ...
    public static function get_instance_config_options($institution, $instance = 0) {
        ...
        $elements = array(
            ...
            'userroleset' => array(
                'type' => 'fieldset',
                'legend' => get_string('userrolesettings', 'auth.ldap'),
                'collapsible' => true,
                'collapsed' => (!self::$default_config['rolefield']),
                'elements' => array(
                    'rolefield' => array(
                        'type'  => 'text',
                        'title' => get_string('ldapfieldforrole', 'auth.ldap'),
                        'defaultvalue' => self::$default_config['rolefield'],
                        'help' => false,
                    ),
                    'adminrolevalue' => array(
                        'type'  => 'text',
                        'title' => get_string('ldapvalueforadminrole', 'auth.ldap'),
                        'defaultvalue' => self::$default_config['adminrolevalue'],
                        'description' => get_string('multivaluecomma', 'auth.ldap'),
                        'help' => false,
                    ),
                    'staffrolevalue' => array(
                        'type'  => 'text',
                        'title' => get_string('ldapvalueforstaffrole', 'auth.ldap'),
                        'defaultvalue' => self::$default_config['staffrolevalue'],
                        'description' => get_string('multivaluecomma', 'auth.ldap'),
                        'help' => false,
                    )
                )
...
function auth_ldap_sync_groups(
    ...
    foreach ($auths as $auth) {
        ...
        if ($userattribute !== null) {
            $instance->set_config('syncgroupsuserattribute', $userattribute);
        }
        //EKAMPUS>
        if ($docreate !== null) {
