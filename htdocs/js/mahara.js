...
function get_string(s) {
    ...
    var i = 0;
    // <EKAMPUS
    // Fix issues with date formats, ie. '%%d.%%m.%%Y'
    return str.replace(/%((%)|s)/g, function (m) { return m[2] || args[i++] || m[1]; });
    // EKAMPUS>
}
...
function globalErrorHandler(data) {
    if (data.returnCode == 1) {
        ...
        // <EKAMPUS
        displayMessage(data.message, 'error', true, arguments[1] || 'messages');
        // EKAMPUS>
    }
}
...
function displayMessage(message, type, hideprevmsg) {
    ...
    // <EKAMPUS
    var msgcontainer = arguments[3] || 'messages';
    var oldmessage = getFirstElementByTagAndClassName('div', null, msgcontainer);
    // EKAMPUS>
    var message = makeMessage(message, type);

    // <EKAMPUS
    appendChildNodes(msgcontainer, message);
    // EKAMPUS>
...
function sendjsonrequest(script, data, rtype, successcallback, errorcallback, quiet, anon) {
    ...
    // <EKAMPUS
    var messagewrapper = arguments[7] || 'messages';
    // EKAMPUS>
    ...
    d.addCallbacks(function (result) {
        ...
        if (!data.error) {
            ...
            // <EKAMPUS
            globalErrorHandler(data, messagewrapper);
            // EKAMPUS>
        }
        if (errtype) {
