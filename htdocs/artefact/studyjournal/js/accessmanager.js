...
define(['jquery-loader'], function (jq) {
    ...
    var add_result_row = function (rowdata, rownumber, globaldata) {
        ...
        if (_table.type === 'group') {
            link = _$('<a></a>').attr('href', rowdata.url).attr('target', '_blank').text(rowdata.name);
        }
    ...
    var render_access_list_item = function (item) {
        ...
        var row = _$('<li/>').append(_$('<div/>').text(item.name)).append(removeButton)
                .append(input)
                .attr('id', type + '-' + item.id);
        var container = _$('#access-added-' + (type === 'group' ? 'groups' : 'institutions'));
