...
function auth_check_required_fields() {
    ...
    /*< EKAMPUS */
    if (!ensure_accepted_terms()) {
        $elements['acceptterms'] = array(
            'type'  => 'radio',
            'title' => get_string('iagreetothetermsandconditions', 'auth.internal'),
            'description' => get_string('registeragreeterms', 'mahara'),
            'rules' => array('required' => true),
            'options' => array(
                'yes' => get_string('yes'),
                'no'  => get_string('no')
...
/*< EKAMPUS */
function ensure_accepted_terms() {
    global $USER;
    if (!get_account_preference($USER->id, 'termsaccepted')){
        return false;
    }
    else {
        return true;
...
function requiredfields_validate(Pieform $form, $values) {
    ...
    if (isset($values['acceptterms']) && $values['acceptterms'] != 'yes') {
        $form->set_error('acceptterms', get_string('youmaynotusewithouttandc', 'mahara'), false);
    }
}
    ...
    foreach ($values as $field => $value) {
        ...
        if ($field == 'email') {
            ...
            if ($field == 'acceptterms'){
                if ($values['acceptterms'] == 'yes'){
                    set_account_preference($USER->id, 'termsaccepted', 1);
                }
                else { //this shouldnt happen because we already validated against it...
                    $USER->logout();
                    die_info(get_string('registeragreeterms', 'mahara', get_config('wwwroot')));
                }
                $otherfield = true;
            }
            else {
...
function auth_draw_login_page($message=null, Pieform $form=null) {
    ...
    $smarty->assign('TRANSIENT_LOGIN', true);
    // EKAMPUS >
...
//                'required'    => true
        ...
        'register' => array(
            ...
            // <EKAMPUS
                . '<a href="#" class="lostusername">' . get_string('lostusernamepassword') . '</a></div>'
            // EKAMPUS>
        ),
    );
    $elements = array(
...
function get_login_form_js($form) {
    ...
    $helptext = get_string('lostusernamepasswordhelp');
    $titletext = get_string('lostusernamepassword');
    // EKAMPUS>
    $js = <<< EOF
...
// <EKAMPUS
\$j.getScript('{$wwwroot}local/js/lib/bootstrap.min.js', function () {
  \$j('#login-helplinks a.lostusername').popover({
    content: '$helptext',
    title: '$titletext',
    ...
    try {
        ...
        try {
            ...
            while ((list(, $authinstance) = each($authinstances)) && false == $authenticated) {
                ...
                //<EKAMPUS
                if (is_callable(array($auth, 'set_user_role'))) {
                    $auth->set_user_role($USER->get('id'), $userdata);
                }
                //EKAMPUS>
            }
...
function auth_generate_login_form() {
    ...
    //<EKAMPUS
    else if (isset($SESSION->loginform)) {
    // EKAMPUS >
