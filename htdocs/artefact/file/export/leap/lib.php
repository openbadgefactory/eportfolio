...
class LeapExportElementFile extends LeapExportElement {
    ...
    public function add_attachments() {
        // <EKAMPUS
        // Show a notification about non-exportable cloudfiles.
        global $SESSION;

        $path = $this->artefact->get_path();

        if (!is_file($path)) {
            $cloud = get_field('artefact_cloudfile', 'type', 'artefact', $this->artefact->get('id'));

            if ($cloud) {
                $SESSION->add_error_msg(get_string('cannotexportcloudfile', 'export', $this->artefact->get('title')));
                return;
            }
        }
        // EKAMPUS>
