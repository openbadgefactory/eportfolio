...
safe_require('interaction', 'learningobject'); // EKAMPUS
...
if ($view->get('type') == 'profile') {
    ...
    define('TITLE', $title); //EKAMPUS . ': ' . get_string('editcontent', 'view'));
}
    ...
    define('TITLE', $view->get('title')); //EKAMPUS. ': ' . get_string('editcontent', 'view'));
    $editabletitle = true;
}
...
// <EKAMPUS
// Show a message if a teacher is editing a learning object thas has already
// been assigned. Also show a message if the user is editing a page that has
// already been returned at some point.
$collection = $view->get_collection();

if ($collection !== false && $collection->get('type') === 'learningobject') {
    if (InteractionLearningobjectInstance::is_assigned($collection)) {
        $SESSION->add_info_msg(get_string('editwarninglearningobjectassigned', 'interaction.learningobject'));
    }
}

// Check whether the collection has already been returned.
if ($collection !== false) {
    $return_date = InteractionLearningobjectInstance::get_previous_collection_return_date($collection);

    if ($return_date !== false) {
        $SESSION->add_info_msg(get_string('editwarningcollectionreturned', 'interaction.learningobject', $return_date));
    }
}

// Check for returned view.
else {
    $return_date = InteractionLearningobjectInstance::get_previous_view_return_date($view);

    if ($return_date !== false) {
        $SESSION->add_info_msg(get_string('editwarningviewreturned', 'interaction.learningobject', $return_date));
    }
}

$javascript[] = get_config('wwwroot') . 'local/js/lib/bootstrap.min.js';
$javascript[] = get_config('wwwroot') . 'local/js/lib/require.js';
$stylesheets = array_merge($stylesheets, array('<link rel="stylesheet" href="' . get_config('wwwroot') . 'local/css/bootstrap.css" />'));
// EKAMPUS>
