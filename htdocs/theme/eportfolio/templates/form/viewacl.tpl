...
{{*          <option value="friend" selected="selected">{{str tag=friends section=view}}</option>*}}
          <option value="group" selected="selected">{{str tag=groups section=view}}</option>
          {{* EKAMPUS> *}}
...
<table id="accesslisttable" class="fr hidden fullwidth hidefocus" tabindex="-1">
  <thead>
    <tr class="accesslist-head">
      <th>&nbsp;</th>
      <th colspan="5"><h3 class="title">{{str tag="accessgrantedtofollowing" section="view"}}</h3></th>
    </tr>
    <tr class="accesslist-head">
...
function renderPotentialPresetItem(item) {
    ...
    if (item.preset) {
        // <EKAMPUS
        attribs = {'class': 'preset ' + item.type};
        // EKAMPUS>
    }
    else if (item['class']) {
    ...
    if (item.type == 'allgroups' || item.type == 'allworkspaces') { // EKAMPUS
        connect(addButton, 'onclick', function() {
            var rows = [];
            // <EKAMPUS
            var items = item.type == 'allgroups' ? myGroups : myWorkspaces;
            forEach(items, function(g) {
            // EKAMPUS>
     ...
     // <EKAMPUS
        ...
        connect(addButton, 'onclick', function() {
            sendjsonrequest('access.json.php', {'type': 'token', 'v': viewid}, 'POST', function(data) {
                renderAccessListItem(data); // EKAMPUS
            });
        });
    }
    // EKAMPUS>
...
function renderAccessListItem(item) {
    ...
    if (item.type == 'token') {
        var sp = item.id.split(':', 2);
        sp[1] = sp[1] || '';

        var tokenmailer = function () {
            var input = this.parentNode.previousSibling.getElementsByTagName('input');
            var body = "{{str tag=token section=view}}:\r\n{{$WWWROOT}}view/view.php?t=" + sp[0] + "\r\n\r\n";
            if (input[0] && input[0].value) {
                body += "{{str tag=password}}:\r\n" + input[0].value + "\r\n\r\n";
            }
            this.href = 'mailto:?body=' + encodeURIComponent(body);
            return true;
        };

        row2 = TR({'class': cssClass},
            TD(null),
            TD({'colspan': 3}, '{{$WWWROOT}}view/view.php?t=' + sp[0]),
            TD(null, LABEL(null,{{jstr tag=password}})),
            TD(null,
                INPUT({
                    'type': 'text',
                    'name': 'accesslist[' + count + '][tokenpassword]',
                    'value': sp[1],
                    'style': 'width: 100px'
                })
            ),
            TD(null, A({'href': '#', 'onclick': tokenmailer}, IMG({src: '{{$WWWROOT}}theme/raw/static/images/usermessage.png', alt: 'email', title: 'email'})))
        );
        item.id = sp[0];
    }
    // KYVYT>
...
// <EKAMPUS
var allWorkspaces = {{$allworkspaces|safe}};
var myWorkspaces = {{$myworkspaces|safe}};

function showGrouplist(groups, title, allitem, morecls) {
    appendChildNodes('potentialpresetitems', H3({'class': 'title'}, title));
    renderPotentialPresetItem(allitem);
    var i = 0;
    var maxGroups = 10;
    forEach(groups, function(preset) {
        if (i == maxGroups) {
            var more = A({'href':''}, {{jstr tag=moregroups section=group}} + ' »');
            connect(more, 'onclick', function(e) {
                e.stop();
                // <EKAMPUS
                forEach(getElementsByTagAndClassName('div', morecls, 'potentialpresetitems'), partial(toggleElementClass, 'hidden'));
                // EKAMPUS>
            });
            appendChildNodes('potentialpresetitems', DIV(null, ' ', more));
        }
        if (i >= maxGroups) {
            preset['class'] = 'hidden ' + morecls; // EKAMPUS
        }
        renderPotentialPresetItem(preset);
        i++;
    });
...
if (myGroups && myGroups.length > 0) {
    showGrouplist(myGroups, {{jstr tag=sharewithmygroups section=view}},
        allGroups, 'moregroups');
}
if (myWorkspaces && myWorkspaces.length > 0) {
    showGrouplist(myWorkspaces, {{jstr tag=sharewithmyworkspaces section=view}},
        allWorkspaces, 'moreworkspaces');
}
// EKAMPUS>
...
searchTable.rowfunction = function(rowdata, rownumber, globaldata) {
    ...
    var buttonTD = TD({'class': 'buttontd ' + rowdata.type}); // EKAMPUS

    var addButton = BUTTON({'type': 'button', 'class': 'button'}, {{jstr tag=add}});
    connect(addButton, 'onclick', function() {
        renderAccessListItem(rowdata); // EKAMPUS
    });
