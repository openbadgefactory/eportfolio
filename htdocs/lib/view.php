...
class View {
    ...
    public function __construct($id=0, $data=null) {
        ...
        // <EKAMPUS
        // $this->columnsperrow is probably initialized if the value is an
        // empty array. No need to get the value again from the db.
        if (empty($this->columnsperrow) && !is_array($this->columnsperrow)) {
        // EKAMPUS>
    ...
    public static function create_from_template($viewdata, $templateid, $userid=null, $checkaccess=true, $titlefromtemplate=false) {
        ...
        // <EKAMPUS
        // Apply the original skin to copied page if the user can use the skin.
        if (can_use_skins()) {
            require_once('skin.php');

            $skinid = $template->get('skin');

            if (!empty($skinid)) {
                $skin = new Skin($skinid);

                if ($skin->can_use()) {
                    $view->set('skin', $skinid);
                }
            }
        }
        // EKAMPUS>
        ...
        if ($blocks) {
            //< EKAMPUS
            $sql = "SELECT blocktype FROM {blocktype_installed_viewtype} WHERE viewtype = ?";
            $allowedblocktypes = get_column_sql($sql, array($view->get('type')));
            // EKAMPUS >
            foreach ($blocks as $b) {
                //< EKAMPUS
                if (!in_array($b->blocktype, $allowedblocktypes)){
                    continue;
                }
                // EKAMPUS >
    ...
    private static function _create(&$viewdata, $userid) {
        ...
        if (isset($viewdata['owner'])) {
            ...
            // <EKAMPUS
            $multipleallowed = array('portfolio', 'studyjournal', 'blog', 'multiresume', 'epsp');

            if (isset($viewdata['type']) && !in_array($viewdata['type'], $multipleallowed) && get_record('view', 'owner', $viewdata['owner'], 'type', $viewdata['type'])) {
            // EKAMPUS>
        ...
        if (!isset($this->artefact_instances)) {
            ...
            if ($instances = $this->get_artefact_metadata()) {
                foreach ($instances as $instance) {
                    safe_require('artefact', $instance->artefacttype);
...
/*< EKAMPUS*/
    public function get_access_records($viewid = null) {
        if ($viewid == NULL){
            $viewid = $this->id;
        }/* EKAMPUS>*/
        $data = get_records_sql_array(
             // <EKAMPUS
                "SELECT accesstype, va.group, va.institution, va.token, role, usr, startdate, stopdate, allowcomments, approvecomments " .
            // EKAMPUS>
            "FROM {view_access} va
            WHERE view = ? AND visible = 1" ./*<EKAMPUS AND token IS NULL EKAMPUS>*/"
            ORDER BY
    ...
    public function copy_access($to) {
        ...
        delete_records_select(
            'view_access',
            // <EKAMPUS
            'view IN (' . join(',', $toupdate) . ') AND visible = 1'
            // EKAMPUS
        );
    ...
    */
    ...
    public function get_default_category() {
        ...
        return is_null($first) ? '' : $first['name'];
        // EKAMPUS>
    }
    ...
    private function get_category_data() {
        ...
        $categories = array();
        // <EKAMPUS
        // To get the general-category fall to the bottom of the list
        $sql = 'SELECT bic.*, (IF(bc.name=?,999,bc.sort)) AS sort FROM {blocktype_installed_category} bic
        ...
        // <EKAMPUS
        $blockcategories = get_records_sql_array($sql, array('general', $this->get('type')));
        // EKAMPUS>
    ...
    public function addblocktype($values) {
        ...
        $bi = new BlockInstance(0,
            array(
                ...
                'order'      => $values['order'],
                // <EKAMPUS
                'configdata' => isset($values['configdata']) ? $values['configdata'] : array()
                // EKAMPUS>
            )
        );
    ...
    */
    ...
    public function get_all_blocktype_css() {
        ...
        // <EKAMPUS
        // array_unique preserves keys -> can end up being an object in JS,
...
//        return array_unique($cssfiles);
    ...
    private function add_blocktype_path($blockinstance, $jsfilename) {
        $pluginname = $blockinstance->get('blocktype');
        // <EKAMPUS
        if (!preg_match('/https?:/', $jsfilename)) {
        // EKAMPUS>
            if ($blockinstance->get('artefactplugin')) {
...
//                    $this->shuffle_cell($values['row'], $currentcol, null, $block->get('order'));
    ...
    public static function import_from_config(array $config, $userid=null, $format='') {
        ...
        // <EKAMPUS
        if (isset($config['approvecomments'])) {
            $viewdata['approvecomments'] = $config['approvecomments'];
        }
        // EKAMPUS>
    ...
    public static function get_myviews_data($limit=5, $offset=0, $query=null, $tag=null, $groupid=null, $institution=null) {
        ...
        // <EKAMPUS
        // Hide the wall from views list.
        $where .= " AND v.type != 'dashboard'";
        // EKAMPUS>
    ...
    public static function get_myviews_url($group=null, $institution=null, $query=null, $tag=null) {
        ...
        if ($group) {
            // <EKAMPUS
            $url = get_config('wwwroot') . 'interaction/pages/grouppages.php';
            // EKAMPUS>
    ...
    public static function owner_sql($ownerobj) {
        /*<EKAMPUS modifying owner_sql if there are multiple owners*/
        if (isset($ownerobj->multiple)) {
            if (isset($ownerobj->group) && is_numeric($ownerobj->group)) {
                //$viewsin = '"group" = ' . (int)$ownerobj->group;
                //select groupmembers from groups institution /*who are not admins and not staff*/
                $viewsin = 'owner IN (SELECT DISTINCT(member) FROM {group_member} gp LEFT JOIN {group} g ON gp."group" = g.id LEFT JOIN usr_institution ui ON g.institution = ui.institution  WHERE ui.usr = gp.member '/*AND ui.staff = 0 AND ui.admin = 0*/.' AND gp."group" ='. (int)$ownerobj->group .')';
                return $viewsin;
            }
            else if (isset($ownerobj->institution)) {
                if (isset($ownerobj->nogroup)){
                    return 'owner IN (SELECT usr FROM {usr_institution}
                    WHERE '/*staff = 0 AND admin = 0 AND */.'institution = ' . db_quote($ownerobj->institution) .')';
                }
                else {
                    //select groups from institutions AND institution members
                    return 'owner IN (SELECT usr FROM {usr_institution}
                    WHERE '/*staff = 0 AND admin = 0 AND */.'institution = ' . db_quote($ownerobj->institution) .') OR v.group IN (SELECT id FROM {group} g WHERE g.deleted = 0 AND g.grouptype !='. db_quote('system') .'AND g.institution =' . db_quote($ownerobj->institution) .')';
                }
            }
            else if (isset($ownerobj->owner)){
                ...
                // If no group and no institution selected
                // -> just check if user is admin in any institution + all groups
                $exclude = "";

                if (isset($ownerobj->exclude_owner) && is_numeric($ownerobj->exclude_owner)) {
                    $exclude = 'WHERE id != ' . (int) $ownerobj->exclude_owner;
                }
                if (isset($ownerobj->nogroup)){
                    $sql = "
                    owner IN
           ...
           if (isset($ownerobj->institution)) {
            ...
            // <EKAMPUS
            if (isset($ownerobj->exclude_owner) && is_numeric($ownerobj->exclude_owner)) {
                return 'owner != ' . (int) $ownerobj->exclude_owner;
            }
            // EKAMPUS>
    ...
    // <EKAMPUS
    public static function view_search_new($query = null, $ownerquery = null,
            $ownedby = null, $copyableby = null, $limit = null, $offset = 0,
            $extra = true, $sort = null, $types = null, $collection = false,
            $accesstypes = null, $tag = null) {
        ...
        // Include normal collections by default or if it is explicitly defined.
        $includecollections = !is_array($types) || in_array('collection', $types);
        // Include learning objects by default or if it is explicitly defined.
        $includelearningobjects = !is_array($types) || in_array('learningobject', $types);
        $search_collections = $includecollections || $includelearningobjects;

        $collwhere = '';
        $collfromparams = array();
        $collwhereparams = array();

        // Remove 'collection' and 'learningobject' from types, those are not
        // real view types.
        foreach (array('collection', 'learningobject') as $type) {
        ...
        $collfrom = "
            ...
            LEFT OUTER JOIN {view} v ON cv.view = v.id
            LEFT OUTER JOIN {usr} qu ON (v.owner = qu.id)
            ";
        ...
        $collwhere .= $where;
        $collwhereparams[] = $viewerid;

        // Get only learning objects (no portfolios).
        if ($includelearningobjects && !$includecollections) {
            $collwhere .= ' AND c.type = ?';
            $collwhereparams[] = 'learningobject';
        }
        // Get only portfolios (no learning objects).
        else if ($includecollections) {
            $where .= ' AND c.type IS NULL';
            $collwhere .= ' AND c.type IS NULL';
        }
        // No portfolios, no learning objects. Do not include views from
        // learning objects.
        else if (!$includelearningobjects) {
            $where .= ' AND c.type IS NULL';
        }
        ...
        // Searching only from collections, no need to include views.
        // PENDING: This is just a quick hack, instead remove the upper part
        //          of the UNION-clause.
        if (is_array($types) && empty($types) && $search_collections) {
            $where .= ' AND FALSE';
        }
        else {
...
//                $where .= ' AND v.type NOT IN (';
            ...
            $typelimit .= join(',', array_map('db_quote', $types)) . ')';
            $where .= ' AND ' . $typelimit;
        }

        if ($ownedby) {
            $ownersql = ' AND (v.' . self::owner_sql($ownedby).')';
            $where .= $ownersql;
            $collwhere .= $ownersql;
        }
        ...
        if ($query) { // Include matches on the title, description or tag
            ...
            $from .= "
                LEFT JOIN {view_tag} vt ON (vt.view = v.id AND vt.tag = ?)";
            $where .= "
                AND (v.title $like '%' || ? || '%' $collate OR v.description $like '%' || ? || '%' $collate OR vt.tag = ? $collate";
            // TODO: view-rajaukset pois portfolioista??
//            $collfrom .= "
//                LEFT JOIN {view_tag} vt ON (vt.view = v.id AND vt.tag = ?)";
//            $collwhere .= "
//                AND (v.title $like '%' || ? || '%' $collate OR v.description $like '%' || ? || '%' $collate OR vt.tag = ? $collate";
//            array_push($collfromparams, $query);
//            array_push($collwhereparams, $query, $query, $query);
            array_push($fromparams, $query);
            array_push($whereparams, $query, $query, $query);
            if ($collection) {
                $where .= "
                    OR c.name $like '%' || ? || '%' $collate OR c.description $like '%' || ? || '%' $collate ";
                array_push($whereparams, $query, $query);
            }

            $collwhere .= "
                    AND (c.name $like '%' || ? || '%' $collate OR c.description $like '%' || ? || '%' $collate ";
//            $collwhere .= "
//                    OR c.name $like '%' || ? || '%' $collate OR c.description $like '%' || ? || '%' $collate ";
            array_push($collwhereparams, $query, $query);
            $collwhere .= ")";
            $where .= ")";
        }
            ...
            if (is_array($tag) && count($tag) > 0) {
                $tagcount = count($tag);
                $tagstr = implode(', ', array_map('db_quote', $tag));
                $where .= "
                    AND v.id IN (
                        SELECT view
                        FROM {view_tag}
        ...
        if ($search_collections) {
            $ph = array_merge($fromparams, $whereparams, $collfromparams, $collwhereparams);
            $sql = "
                (SELECT v.id, 'view' AS type $from $where)
                    UNION
                (SELECT c.id, 'collection' AS type $collfrom $collwhere)";
        }
        else {
            $ph = array_merge($fromparams, $whereparams);
            $sql = "SELECT v.id, 'view' AS type $from $where";
        }
        ...
        $views_and_collections = get_records_sql_array($sql, $ph);

        if (empty($views_and_collections)) {
            return $result;
        }

        $matching_views = array();
        $matching_collections = array();

        foreach ($views_and_collections as $item) {
            if ($item->type === 'view') {
                $matching_views[] = $item->id;
            }
            else {
                $matching_collections[] = $item->id;
            }
        }

        $accesstypes = !is_array($accesstypes) ? array() : $accesstypes;
        $own_views_and_collections = self::get_available_views_and_collections($accesstypes,
                $ownedby, $typelimit);

        $views = array_intersect($matching_views, $own_views_and_collections['views']);
        $collections = array_intersect($matching_collections, $own_views_and_collections['collections']);

        if ($ownerquery) {
            $owner_views_and_collections = self::get_owners_views_and_collections($ownerquery);
            $views = array_intersect($views, $owner_views_and_collections['views']);
            $collections = array_intersect($collections, $owner_views_and_collections['collections']);
        }

        $total = count($views) + count($collections);
        ...
        if (!empty($sort)) {
            ...
            ';
                    $orderby .= $search_collections ? 'GREATEST(COALESCE(lastcomment, modtime), modtime)' : 'GREATEST(COALESCE(lastcomment, v.mtime), v.mtime)';
                    $getcomments = true;
                    // EKAMPUS>
                }
                    ...
                    $from .= '
                        LEFT OUTER JOIN {usr} su ON su.id = v.owner
            ...
            ';
                    ...
                    // <EKAMPUS
                    if ($search_collections) {
                        $orderby .= 'TRIM(' .(!empty($item['tablealias']) ? $item['tablealias'] . '.' : '') .  $item['column'] .')';
                    }
                    else {
                        $orderby .= 'TRIM(' .(!empty($item['tablealias']) ? $item['tablealias'] : 'v') . '.' . $item['column'].')';
                    }
                    // EKAMPUS>
                }
        ...
        if ($search_collections) {
            $extracols = $getcomments ? ', l.lastcomment' : '';
            $unionparts = array();

            if (count($views) > 0) {
                $viewstr = implode(',', $views);
                $unionparts[] = "
                (
                    SELECT
                        CONCAT('view-', v.id) AS uniqueid, v.id, v.id AS viewid, v.title, v.description, v.owner, v.ownerformat, v.group, v.institution,
                        v.template, v.mtime, v.ctime,
                        c.id AS collid, c.name, v.type, v.urlid, v.mtime AS modtime
                        $extracols
                    FROM {view} v
               LEFT JOIN {collection_view} cv ON cv.view = v.id
               LEFT JOIN {collection} c ON cv.collection = c.id
                    $from
                    WHERE v.id IN ($viewstr)
                )";
            }

            if (count($collections) > 0) {
                $collectionstr = implode(',', $collections);
                $unionparts[] = "(
                    SELECT
                        CONCAT('coll-', c.id) AS uniqueid, v.id, v.id AS viewid,
                        c.name AS title, c.description, c.owner, v.ownerformat,
                        c.group, c.institution, v.template, c.mtime, c.ctime,
                        c.id AS collid, c.name, 'collection' AS type, v.urlid,
                        v.mtime AS modtime
                        $extracols
                    FROM {collection} c
                    LEFT OUTER JOIN (
                        SELECT collection, view, displayorder AS dorder
                        FROM {collection_view} cv1
                        WHERE displayorder = (
                            SELECT MIN(displayorder)
                            FROM {collection_view} AS cv2
                            WHERE cv2.collection = cv1.collection
                        )
                    ) cv ON cv.collection = c.id
                    LEFT JOIN {view} v ON cv.view = v.id
                    $collfrom
                    WHERE c.id IN ($collectionstr)
                )";
            }

            $sql = implode(' UNION ', $unionparts);
            $viewdata = get_records_sql_assoc("$sql ORDER BY $orderby, viewid ASC", array(), $offset, $limit);
//            $count = count_records_sql("SELECT COUNT(*) FROM ($sql) AS tbl", $ph);
        }
        else {
            if (count($views) > 0) {
//            $count = count_records_sql('SELECT COUNT(*) ' . $from . $where, $ph);
                ...
                $viewdata = get_records_sql_assoc("
                    SELECT
                        v.id, v.id AS viewid, v.title, v.description, v.owner,
                        v.ownerformat, v.group, v.institution, v.template, v.mtime, v.ctime,
        ...
        if ($viewdata) {
            if ($extra) {
                View::get_extra_view_info($viewdata, false, true, $search_collections);
            }
    ...
    public static function get_owners_views_and_collections($ownerquery) {
        $like = db_ilike();
        $viewsql = "SELECT v.id FROM {view} v LEFT OUTER JOIN {usr} qu ON (v.owner = qu.id)";
        $collectionsql = "SELECT c.id FROM {collection} c LEFT OUTER JOIN {usr} qu ON (c.owner = qu.id)";
        $from = "
        ...
        $params = array($ownerquery, $ownerquery, $ownerquery, $ownerquery, $ownerquery, $ownerquery);
        $owner_views = get_column_sql($viewsql . $from . $where, $params);
        $owner_collections = get_column_sql($collectionsql . $collectionfrom . $where, $params);

        return array(
            'views' => empty($owner_views) ? array() : $owner_views,
            'collections' => empty($owner_collections) ? array() : $owner_collections
        );
    }
     ...
     *
     * @global type $USER
     * @param type $accesstypes
     */
    public static function get_available_views_and_collections($accesstypes = array(),
            $ownedby = null, $typelimit = '') {
        global $USER;

        $uid = (int) $USER->get('id');
        $available = array('views' => array(), 'collections' => array());

        // Prefetch user's groups to speed up view queries later.
        $groups = get_column_sql("
            SELECT m.group
              FROM {group_member} m
              JOIN {group} g ON m.member = ? AND m.group = g.id
             WHERE m.role = 'admin' OR g.editroles = 'all'
                OR (g.editroles != 'admin' AND m.role != 'member')", array($uid));

        $grouppart = empty($groups) ? "" : "OR v.group IN (" . implode(',', $groups) . ")";
        $isownersql = "
            (v.owner = $uid $grouppart)";

        $currently_shared = "(NOW() BETWEEN COALESCE(va.startdate, NOW()) AND COALESCE(va.stopdate, NOW()))";
        $shared_now = "(
            (NOW() BETWEEN COALESCE(va.startdate, NOW()) AND COALESCE(va.stopdate, NOW())) AND
            (NOW() BETWEEN COALESCE(v.startdate, NOW()) AND COALESCE(v.stopdate, NOW()))
            )";

        $published_accesstypes = array('user', 'group', 'institution',
            'loggedin', 'token');
        ...
        if ($ownedby) {
            $ownersql = '(v.' . self::owner_sql($ownedby).')';
        }
        ...
        // Get own public views.
        if ($getpublic) {
            $sharedsql[] = "(va.accesstype = 'public' AND $currently_shared)";
        }

        // Get own published views.
        if ($getpublished) {
            $sharedsql[] = "
                (
        ...
        // Get own (or current group's) private views.
        if ($getprivate) {
            $sharedsql[] = "
                -- Get own views with no records in view_access
                (
                    v.id NOT IN (
                        SELECT va.view
                          FROM {view_access} va
                ...
                (
                    v.id IN (
                        SELECT va.view
                         ...
                         WHERE
                         (
                            (va.stopdate IS NOT NULL AND va.stopdate < NOW())
                            OR
                            (va.startdate IS NOT NULL AND va.startdate > NOW())
                         )
                    ) AND $ownersql
                )
                ";
        ...
        $queries = array();
        $typelimit_sql = empty($typelimit) ? '' : $typelimit . ' AND';
        $select = "
            SELECT v.id, c.collection
              FROM {view} v
         LEFT JOIN {collection_view} c ON v.id = c.view
         LEFT JOIN {view_access} va ON va.view = v.id";

        // User or user's group is the owner.
        $queries['own'] = $queries['owngroup'] = "
            $select WHERE $typelimit_sql $isownersql" .
            (count($sharedsql) > 0 ? " AND (" . implode(" OR ", $sharedsql) .
            ")" : "") . " AND $timestr";

        $queries['private'] = "
            $select WHERE $typelimit_sql v.id IS NULL OR v.id NOT IN (
                SELECT va.view
                FROM {view_access} va
            ) AND $timestr";

        $queries['public'] = "
            $select WHERE $typelimit_sql va.accesstype = 'public' AND ($isownersql OR $shared_now) GROUP BY v.id";

        $queries['loggedin'] = "
            $select WHERE $typelimit_sql va.accesstype = 'loggedin' AND ($isownersql OR $shared_now) GROUP BY v.id";

        $queries['user'] = "
            $select WHERE $typelimit_sql (va.usr = $uid AND $shared_now) OR (va.usr IS NOT NULL AND $isownersql)
                GROUP BY v.id";

        // Shared to own groups.
        $queries['group'] = "
            $select LEFT JOIN {group_member} m ON va.group = m.group
                WHERE $typelimit_sql (va.role = m.role OR va.role IS NULL) AND m.member = $uid
                    AND $shared_now
                GROUP BY v.id";

        // Shared to own institutions.
        // PENDING: Search also own views shared to institutions?
        $queries['institution'] = "
            $select LEFT JOIN {usr_institution} ui ON va.institution = ui.institution
                WHERE $typelimit_sql ui.usr = $uid AND $shared_now
                GROUP BY v.id";

        // Shared with a token by the current user/group.
        $queries['token'] = "
            $select WHERE $typelimit_sql (va.token IS NOT NULL AND $isownersql) GROUP BY v.id";

        if (is_array($accesstypes)) {
//            $editableviews = in_array('editable', $accesstypes);
        }
        else if ($uid) {
            $accesstypes = array('public', 'loggedin', 'user', 'group', 'institution');
        }
        else {
            $accesstypes = array('public');
        }

        $supersqlqueries = array();

        foreach ($accesstypes as $t) {
            // Get group views only in group section.
            if ($t === 'owngroup' && empty($group)) {
                continue;
            }
            ...
            $supersqlqueries[] = $queries[$t];
        }

        if (count($supersqlqueries) > 0) {
            $views = get_records_sql_array(implode(' UNION ', $supersqlqueries), array());

            if (is_array($views)) {
                foreach ($views as $item) {
                    $available['views'][] = $item->id;

                    if (!empty($item->collection)) {
                        $available['collections'][] = $item->collection;
                    }
                }

                $available['views'] = array_unique($available['views']);
                $available['collections'] = array_unique($available['collections']);
            }
        }
    ...
    public static function view_search($query=null, $ownerquery=null, $ownedby=null, $copyableby=null, $limit=null, $offset=0,
        ...
        return self::view_search_new($query, $ownerquery, $ownedby, $copyableby,
                $limit, $offset, $extra, $sort, $types, $collection,
                $accesstypes, $tag);
        ...
        $collfrom = "
            FROM {collection} c
            LEFT OUTER JOIN (SELECT collection, view, displayorder AS dorder FROM {collection_view} cv1 WHERE displayorder = (
                SELECT MIN(displayorder)
                  FROM {collection_view} AS cv2 WHERE cv2.collection = cv1.collection
            )) cv ON cv.collection = c.id
            LEFT OUTER JOIN {view} v ON cv.view = v.id
            LEFT OUTER JOIN {usr} qu ON (v.owner = qu.id)
            ";
        ...
        foreach ($accesstypes as $t) {
            ...
            if ($t === 'own' || ($t === 'owngroup' && !is_null($group))) {
                ...
                $accesssql[] = "v.id IS NULL OR v.id NOT IN ( -- private access
                                SELECT va.view
                                FROM {view_access} va
                            )";
            }
        ...
        $where .= " AND ($editablesql OR $accesssql)";

        // <EKAMPUS
        $collwhere .= " AND ($editablesql OR $accesssql)";

        //if (!$ownedby && $ownerquery) { --> now that ownedby can be multiple users it is sensible to have both ownedby and ownerquery filters at the same time
        if ($ownerquery){
        // EKAMPUS>
            $from .= '
            LEFT OUTER JOIN {group} qg ON (v.group = qg.id)
            ...
            $collwhere .= $ownerwhere;
            $where .= $ownerwhere;
            $collwhereparams = array_merge($collwhereparams, array($ownerquery,$ownerquery,$ownerquery,$ownerquery,$ownerquery));
            // EKAMPUS
        ...
        if (!empty($sort)) {
            ...
            LEFT OUTER JOIN {institution} si ON si.name = v.institution
            ';
                    $collfrom .= 'LEFT OUTER JOIN {usr} su ON su.id = v.owner
            LEFT OUTER JOIN {group} sg ON sg.id = v.group
            LEFT OUTER JOIN {institution} si ON si.name = v.institution
            ';
                    $orderby .= "COALESCE(sg.name, si.displayname, CASE WHEN su.preferredname IS NOT NULL AND su.preferredname != '' THEN su.preferredname ELSE su.firstname || ' ' || su.lastname END)";
        ...
        if ($search_collections) {
            ...
            $sql = "
                ...
                (
                    SELECT
                        CONCAT('coll-', c.id) AS uniqueid, v.id, v.id AS viewid, c.name AS title, c.description, c.owner, v.ownerformat, c.group, c.institution,
                        v.template, c.mtime, c.ctime,
                        c.id AS collid, c.name, 'collection', v.urlid, v.mtime AS modtime
                        $extracols
                    $collfrom
                    $collwhere
                )";
            $viewdata = get_records_sql_assoc("$sql ORDER BY $orderby, viewid ASC", $ph, $offset, $limit);
            $count = count_records_sql("SELECT COUNT(*) FROM ($sql) AS tbl", $ph);
        }
        else {
            $ph = array_merge($fromparams, $whereparams);
            ...
            $viewdata = get_records_sql_assoc('
                SELECT
                    v.id, v.id AS viewid, v.title, v.description, v.owner, v.ownerformat, v.group, v.institution,
                    v.template, v.mtime, v.ctime,
                    c.id AS collid, c.name, v.type, v.urlid
                ' . $from . $where . '
                ORDER BY ' . $orderby . ', v.id ASC',
    ...
    public static function shared_to_user($query=null, $tag=null, $limit=null, $offset=0, $sort='lastchanged', $sortdir='desc',
        $accesstypes=null/*<EKAMPUS*/, $types=array('portfolio'), $ownedby=null, $ownerquery = null/*EKAMPUS>*/) {
        ...
        $result = self::view_search(
            $query, /*<EKAMPUS*/$ownerquery, $ownedby,/*EKAMPUS>*/ null, $limit, $offset, true, $sort,/*<EKAMPUS*/ $types,/*EKAMPUS>*/ false, $accesstypes, $tag
        );
        ...
        // <EKAMPUS
        // Doesn't currently work, $result->ids are in format 'view-10071','coll-175' etc.
        // EKAMPUS>
    ...
    public static function get_extra_view_info(&$viewdata, $getartefacts=true, $gettags=true, $collections_included = false) {
        if ($viewdata) {
            ...
            if (!$collections_included) {
                $viewidlist = join(',', array_map('intval', array_keys($viewdata)));
            }
            else {
                $viewidarr = array();
                foreach (array_keys($viewdata) as $id) {
                    if (strpos($id, 'view-') !== false) {
                        $viewidarr[] = (int) str_replace('view-', '', $id);
                    }
                }
                $viewidlist = join(',', $viewidarr);
            }

            if ($getartefacts && !empty($viewidlist)) {
            // EKAMPUS>
                ...
                if ($artefacts) {
                    foreach ($artefacts as $artefactrec) {
                        ...
                        if (strlen($artname)) {
                            // <EKAMPUS
                            $key = $collections_included ? 'view-' . $artefactrec->view : $artefactrec->view;
//                            $viewdata[$artefactrec->view]->artefacts[] = array('id'    => $artefactrec->artefact,
                            $viewdata[$key]->artefacts[] = array('id'    => $artefactrec->artefact,
                                                                               'title' => $artname);
                            // EKAMPUS>
                        }
                    }
            ...
            // EKAMPUS>
                ...
                if ($tags) {
                    foreach ($tags as &$tag) {
                        // <EKAMPUS
                        $key = $collections_included ? 'view-' . $tag->view : $tag->view;
                        $viewdata[$key]->tags[] = $tag->tag;
                        // EKAMPUS>
                    }
                }
            }
            ...
            // <EKAMPUS
            // Get rows/columns now so that they won't be fetched in the View's
            // constructor.
            if (!empty($viewidlist)) {
                $rowscols = get_records_select_array('view_rows_columns', 'view IN (' . $viewidlist . ')');

                if (is_array($rowscols)) {
                    foreach ($rowscols as $row) {
                        $key = $collections_included ? 'view-' . $row->view : $row->view;

                        if (!isset($viewdata[$key]->columnsperrow)) {
                            $viewdata[$key]->columnsperrow = array();
                        }

                        $viewdata[$key]->columnsperrow[$row->row] = array('row' => $row->row, 'columns' => $row->columns);
                    }
                }
            }
            // EKAMPUS>
    ...
    public function copy_contents($template) {
        ...
        if ($blocks) {
            //< EKAMPUS dont copy if the block is not installed for this viewtype
            $sql = "SELECT blocktype FROM {blocktype_installed_viewtype} WHERE viewtype = ?";
            $allowedblocktypes = get_column_sql($sql, array($this->get('type')));
            // EKAMPUS >
        ...
        if ($new) {
            ...
            if ($this->get('group')) {
                if ($this->get('type') == 'grouphomepage') {
                    ...
                    // <EKAMPUS
                    $redirecturl = '/interaction/pages/grouppages.php?group='.$this->get('group');
                    // EKAMPUS>
                }
            }
     ...
     */
    // <EKAMPUS
    function get_views_and_collections($owner=null, $group=null, $institution=null, $matchconfig=null, $includeprofile=true, $submittedgroup=null, $types=null) {
    // EKAMPUS>
        ...
        if (is_array($types)) {
            $typesql = implode(',', array_map("db_quote", $types));
        }
        else {
            $typesql = "'portfolio','studyjournal','multiresume','blog','grouphomepage','epsp'" . ($includeprofile ? ",'profile'" : '');
        }
        // EKAMPUS>
        ...
        $sql = "
            SELECT v.id, v.type, v.title, v.accessconf, v.ownerformat, v.startdate, v.stopdate, v.template,
                ...
                c.submittedgroup AS csubmitgroup, c.submittedhost AS csubmithost, c.type AS col_type, " . // EKAMPUS
                db_format_tsfield('c.submittedtime', 'csubmittime') . "
            FROM {view} v
                LEFT JOIN {collection_view} cv ON v.id = cv.view
                LEFT JOIN {collection} c ON cv.collection = c.id
            WHERE  v.type IN ($typesql) "; // EKAMPUS
    ...
    public static function get_accesslists($owner=null, $group=null, $institution=null,
        ...
        $data = array();
        // <EKAMPUS
        list($data['collections'], $data['views']) = self::get_views_and_collections($owner, $group, $institution, null, true, null, $types);
        // EKAMPUS>
...
//            }
    ...
    public function submit($group) {
        ...
        //< EKAMPUS dont send message about system group
...
function create_view_form($group=null, $institution=null, $template=null, $collection=null,
    ...
    if (!empty($addtocollection)) {
        $form['elements']['addtocollection'] = array(
            'type' => 'hidden',
...
function createview_submit(Pieform $form, $values) {
    ...
    if (!empty($values['submitcollection'])) {
        ...
        // <EKAMPUS
        // Add this view to selected collection.
        if (!empty($values['addtocollection'])) {
            $collection = new Collection($values['addtocollection']);

            if ($USER->can_edit_collection($collection)) {
                $views = array('view_' . $view->get('id') => true);
                $collection->add_views($views);

                // Learning objects are copyable by default.
...
function createview_cancel_submit(Pieform $form, $values) {
    if (isset($values['group'])) {
        // <EKAMPUS
        redirect(get_config('wwwroot') . 'interaction/pages/grouppages.php?group=' . $values['group']);
        // EKAMPUS>
    }
