...
if ($folderid === 0) {
    if (function_exists('zip_open')) {
        ...
        // <EKAMPUS
        // Cloud folders are possibly very large. Skip these for now.
        $from  .= " LEFT JOIN {artefact_cloudfile} af ON a.id = af.artefact ";
        $where .= " AND af.artefact IS NULL ";
        // EKAMPUS>
    ...
    // <EKAMPUS
    if (get_field('artefact_cloudfile', 'artefact', 'artefact', $folderid)) {
        $smarty = smarty();
        $smarty->assign('message', get_string('folderdownloaddisabled','artefact.cloudfile'));
