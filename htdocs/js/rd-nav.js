...
$j(document).ready(function(){
    /* < EKAMPUS Disable responsive nav on non-handheld devices. -- make it bigger than 600px*/
    if (!window.config.handheld_device && $j(window).width() > 600) {
        return;
    }
    /* EKAMPUS >*/
...
//    $j(".rd-nav-title a").click(function(event) {
   ...
        $j(".main-nav").toggleClass("nav-expand");
        // < EKAMPUS if there is rd-nav-title --> dont display dropdown nav
        $j(".main-nav #dropdown-nav").attr("id", "nav");
        $j(".main-nav .dropdown-sub").removeClass("dropdown-sub");
        // EKAMPUS >
