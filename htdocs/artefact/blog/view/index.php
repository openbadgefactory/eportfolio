...
function changepoststatus_success(form, data) {
    if ($('changepoststatus_' + data.id + '_currentpoststatus').value == 0) {
        ...
        // <EKAMPUS
        // Do not show 'published'-text to prevent user confusion (ie. blog
        // is not published, but the single entry is)
        //$('poststatus' + data.id).innerHTML = {$strpublished};
        $('poststatus' + data.id).innerHTML = '';
        // EKAMPUS>
...
$createblogform = ArtefactTypeBlog::create_blog_form($id, $config);
if (isset($createblogform['elements']['blogviewid'])) {
    $smarty->assign('blogviewid', (string)$createblogform['elements']['blogviewid']['value']);
}
$createblogform = pieform($createblogform);
$smarty->assign('createblogform', $createblogform);
// EKAMPUS >
