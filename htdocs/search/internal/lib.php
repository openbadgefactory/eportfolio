...
class PluginSearchInternal extends PluginSearch {
    ...
    public static function search_user($query_string, $limit, $offset = 0, $data=array()) {
        ...
        // <EKAMPUS
        // Search for teachers in own institutions only.
        if (!empty($data['teachers'])) {
            $where .= ' AND u.id IN (
                SELECT usr
                  FROM {usr_institution}
                 WHERE (staff = 1 OR admin = 1) AND institution IN (
                    SELECT institution
                      FROM {usr_institution}
                     WHERE usr = ' . $USER->get('id') . '
                    )
                )';
        }
        // EKAMPUS>
    ...
    private static function prepare_search_user_options($options) {
        ...
        if (isset($options['group'])) {
            ...
            // <EKAMPUS
            // Allow teachers to see the members in the institution's groups.
            $group_institution = get_group_institution($options['group']);
            $is_teacher_in_groupinstitution = is_teacher_in($group_institution);
            // EKAMPUS>
            if ($USER->get('admin') || $USER->get('staff')
                // <EKAMPUS
                || $is_teacher_in_groupinstitution
                // EKAMPUS>
    ...
    public static function search_group($query_string, $limit, $offset=0,
            $type='member', $category='', $includesystemgroups = true,
            $onlysystemgroups = false) {
    // EKAMPUS>
        global $USER;
        $data = array();
