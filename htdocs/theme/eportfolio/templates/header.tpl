...
<body class="no-js {$CURRENTPAGE}{if $LOGGEDOUT && !$TRANSIENT_LOGIN} logged-out{/if} {$BODYCLASSES}">
...
{include file="header/topright.tpl"}
 {if $LOGINBLOCK && !$HIDE_LOGINBLOCK}
            <div id="loginform_container">{$LOGINBLOCK|safe}</div>
        {/if}
        </div>
...
{if isset($PAGEHEADING)}                    <h1><span class="pageheading">{$PAGEHEADING|safe}</span>{if $PAGEHELPNAME}<span class="page-help-icon">{$PAGEHELPICON|safe}</span>{/if}</h1>
{/if}
