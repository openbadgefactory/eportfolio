...
abstract class PluginBlocktype extends Plugin {
    ...
    */
    ...
    // <EKAMPUS
    /**
     * Override this to return false, if the blocktype will create a copy
     * instead of a reference when copying a block for the same user.
     *
     * @return boolean Return false to create a copy, true otherwise.
     */
    public static function create_sameowner_reference() {
        return true;
    }

    /**
     * Enable blocktype plugins to define their own config form renderers.
     *
     * @return string The form renderer name.
     */
    public static function get_instance_config_form_renderer() {
        return 'maharatable';
    }
    // EKAMPUS>
    ...
    */
    ...
    public static function get_blocktypes_for_category($category, View $view) {
        // <EKAMPUS
        // CASE-structure to sort the blocktypes inside categories.
        $sql = 'SELECT bti.name, bti.artefactplugin, (
            CASE bti.name
                WHEN \'navigation\' THEN 1
                WHEN \'entiremultiresume\' THEN 1
                WHEN \'comment\' THEN 2
                WHEN \'singleresumefield\' THEN 2
                WHEN \'recentforumposts\' THEN 3
                WHEN \'multiresumefield\' THEN 3
                WHEN \'creativecommons\' THEN 4
                WHEN \'othersrecentposts\' THEN 100
                ELSE 99
            END) AS sort
...
class BlockInstance {
    ...
    public function render_viewing() {
        ...
        // <EKAMPUS
        if (!empty($configdata['titlelinkurl'])) {
                $smarty->assign('titlelinkurl', get_config('wwwroot'). $configdata['titlelinkurl']);
        }
        //EKAMPUS >
...
// EKAMPUS>
        ...
        $form = array(
            'name' => 'instconf',
            // <EKAMPUS
            'renderer' => call_static_method($blocktypeclass, 'get_instance_config_form_renderer'),
            // EKAMPUS>
...
// EKAMPUS>
    ...
    public function copy(View $view, View $template, &$artefactcopies) {
        ...
        // <EKAMPUS
        // Make possible for the blocktype class to always make a copy, even
        // when copying from the same user.
        $create_reference = call_static_method($blocktypeclass, 'create_sameowner_reference');
        if (($sameowner && $create_reference) || $copytype == 'reference') {
        // EKAMPUS>
        ...
        // <EKAMPUS
        // Skip files and folders in cloud
        $prune_cloud_artefacts = function ($ids) {
            if (empty($ids)) {
                return array();
            }
            $placeholder = join(',', array_fill(0, count($ids), '?'));
            $in_cloud = get_column_sql("SELECT artefact from {artefact_cloudfile} WHERE artefact IN ($placeholder)", $ids);
            return array_diff($ids, $in_cloud);
        };
        //$artefactids = get_column('view_artefact', 'artefact', 'block', $this->get('id'));
        $artefactids = $prune_cloud_artefacts( get_column('view_artefact', 'artefact', 'block', $this->get('id')) );
        // EKAMPUS>
        ...
        if (!empty($artefactids)
            ...
            require_once(get_config('docroot') . 'artefact/lib.php');
            $descendants = $prune_cloud_artefacts( artefact_get_descendants($artefactids) );
            // EKAMPUS>
            ...
            foreach ($descendants as $d) {
                if (!isset($artefactcopies[$d])) {
                    ...
                    $attachmentlists[$d] = $prune_cloud_artefacts( $tocopy[$d]->attachment_id_list() );
                    // EKAMPUS>
            ...
            if (isset($configdata['artefactid']) && isset($artefactcopies[$configdata['artefactid']])) { // <EKAMPUS
                $configdata['artefactid'] = $artefactcopies[$configdata['artefactid']]->newid;
            }
            // <EKAMPUS
//            else {
            else if (!empty($configdata['artefactids'])) {
                $newids = array();
                foreach ($configdata['artefactids'] as $oldid) {
                    if (isset($artefactcopies[$oldid])) {
                        $newids[] = $artefactcopies[$oldid]->newid;
                    }
                }
                $configdata['artefactids'] = $newids;
            }
            // EKAMPUS>
        }
        else {
