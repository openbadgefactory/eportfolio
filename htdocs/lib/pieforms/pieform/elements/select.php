...
function pieform_element_select(Pieform $form, $element) {
    ...
    // <EKAMPUS
    // Empty select, why not?
    if (empty($options) && empty($element['allowempty'])) {
    // EKAMPUS
...
function pieform_element_select_set_attributes($element) {
    ...
    // <EKAMPUS
    // Sometimes we really don't want automatic validation, dammit.
    if (empty($element['allowother']) && empty($element['donotvalidateoptions'])) {
    // EKAMPUS>
...
//        $options = array_merge($options, $optgroup['options']);
        // <EKAMPUS
        $options = array_merge_keep_keys($options, $optgroup['options']);
        // EKAMPUS>
    }
