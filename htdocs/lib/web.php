...
function smarty($javascript = array(), $headers = array(), $pagestrings = array(), $extraconfig = array()) {
    ...
    // <EKAMPUS
    // Custom TinyMCE-plugins, formatted as follows:
    //
    // $extraconfig['tinymceplugins'] = array(
    //     array(
    //         'name' => 'mypluginname',
    //         'button' => array(
    //             'row' => 0, // The row for the plugin button
    //             'after' => 'emotions' // Button will be placed after this button.
    //         )
    //     )
    // );
    if (!isset($extraconfig['tinymceplugins'])) {
        $extraconfig['tinymceplugins'] = array();
    }
    // EKAMPUS>
...
//    if ($SESSION->get('handheld_device') == false) {     // <EKAMPUS - Enable TinyMCE on mobile devices.
        ...
        foreach ($checkarray as &$check) {
            if (($key = array_search('tinymce', $check)) !== false || ($key = array_search('tinytinymce', $check)) !== false) {
                if (!$found_tinymce) {
                    ...
                    // <EKAMPUS
                    // Custom plugins for TinyMCE.
                    $pluginstr = '';

                    foreach ($extraconfig['tinymceplugins'] as $plugin) {
                        $pluginstr .= ',' . $plugin['name'];

                        if (isset($plugin['button']) && isset($adv_buttons[$plugin['button']['row']])) {
                            $rowindex = $plugin['button']['row'];
                            $row = $adv_buttons[$rowindex];
                            $buttons = explode(',', $row);

                            array_splice($buttons, array_search($plugin['button']['after'], $buttons) + 1, 0, $plugin['name']);

                            $adv_buttons[$rowindex] = implode(',', $buttons);
                        }
                    }
                    // EKAMPUS>
    ...
    mode: "none",
    theme: "advanced",
    plugins: "table,emotions,inlinepopups,paste,fullscreen{$spellchecker}{$pluginstr}", // EKAMPUS
...
// <EKAMPUS
function ourFileBrowser (field_name, url, type, win) {

    var cmsURL = '$wwwroot/local/filebrowser.php';
    if (cmsURL.indexOf("?") < 0) {
...
function custom_urlconvert (u, n, e) {
  ...
  // <EKAMPUS
  if (s.relative_urls) {
      u = t.documentBaseURI.toRelative(u);
      var re = new RegExp('.*://.*/');
      u = u.replace(re, '/');
  }
  // EKAMPUS>
...
EOF;
    ...
    // <EKAMPUS - Enable TinyMCE on mobile.
//    }
//    else {
...
//    }
    ...
    foreach ($javascript as $jsfile) {
        ...
        // <EKAMPUS
        else if (preg_match("/https?:\/\//", $jsfile) === 0) {
        // EKAMPUS>
    ...
    if (defined('SECTION_PLUGINTYPE') && defined('SECTION_PLUGINNAME') && SECTION_PLUGINTYPE != 'core') {
        if ($pluginsheets = $THEME->get_url('style/style.css', true, SECTION_PLUGINTYPE . '/' . SECTION_PLUGINNAME)) {
            // <EKAMPUS
            $stylesheets = array_merge($stylesheets, array_reverse(array_values($pluginsheets)));
            // EKAMPUS>
        }
    }
    ...
    if ($adminsection = in_admin_section()) {
        if ($adminsheets = $THEME->get_url('style/admin.css', true)) {
            // <EKAMPUS
            // Admin stylesheet overrides raw plugin styles. Test that this
            // doesn't break anything.
            $stylesheets = array_merge($stylesheets, array_reverse(array_values($adminsheets)));
            // EKAMPUS>
        }
    }
    ...
    // <EKAMPUS
    $langcode = substr(current_language(), 0, 2);
    $smarty->assign('LANGUAGE', $langcode);
    // EKAMPUS>
    ...
    $smarty->assign('sitelogo', $THEME->header_logo($langcode)); // EKAMPUS
    ...
    if ($USER->is_logged_in()) {
        ...
        /*< EKAMPUS*/
        elseif (!$mainnavsubnav && !$dropdownmenu){
            //handheld device might not have either of them if we have responsive menu
            $smarty->assign('SUBPAGENAV', $SELECTEDSUBNAV);
        }
        /* EKAMPUS >*/
        else {
...
//    $sidebars = !isset($extraconfig['sidebars']) || $extraconfig['sidebars'] !== false;
    $sidebars = isset($extraconfig['sidebars']) && $extraconfig['sidebars'] === true;
    // EKAMPUS>
...
//        }
    ...
    if (!$USER->is_logged_in() && !(get_config('siteclosed') && get_config('disablelogin'))) {
        $smarty->assign('LOGINBLOCK', auth_generate_login_form());
    }

    // Allow artefact plugins to do extra stuff here.
    if (get_config('installed')) {
        if ($plugins = plugins_installed('artefact')) {
            foreach ($plugins as &$plugin) {
                if (safe_require_plugin('artefact', $plugin->name)) {
                    $clsname = generate_class_name('artefact',$plugin->name);
                    if (method_exists($clsname, 'before_page_render')) {
                        call_static_method($clsname, 'before_page_render');
                    }
                }
            }
        }
    ...
    //< EKAMPUS adding current url to be able to track which page we are on
    $currenturl = strtolower($_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME']);
    $wroot = explode('//', strtolower($wwwroot));
    $link = explode($wroot[1], $currenturl);
    $link = explode('.', $link[1]);
    $currentpage = str_replace('/', '_', $link[0]);
    $smarty->assign('CURRENTPAGE', $currentpage);

    if (isset($extraconfig['bodyclasses']) && is_array($extraconfig['bodyclasses'])) {
        $smarty->assign('BODYCLASSES', implode(' ', $extraconfig['bodyclasses']));
    }
...
function find_menu_children(&$menu, $path) {
    ...
    foreach ($menu as $key => $item) {
        $item['path'] = isset($item['path']) ? $item['path'] : '';
