...
// <EKAMPUS
Shuffle.prototype.paginate = function (sortOpts, opts, exclude_selector) {
    var from = opts.page * opts.limit;
    var to = from + opts.limit;
    var items = $(this.$items.filter('.filtered').sorted(sortOpts));
    var filtered = items.not(exclude_selector).slice(from, to);
    var hidden = items.not(exclude_selector).not(filtered);

    this._toggleFilterClasses(items, hidden);

    // Shrink each concealed item
    this._shrink(hidden);

    // If given a valid sort object, save it so that _reLayout() will sort the items
    this.lastSort = sortOpts;
