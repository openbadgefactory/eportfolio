...
// <EKAMPUS
// Get results only when the search query is given and we're searching
// users who are not currently members:
//
// 'request' = users who have requested membership
// 'invite' = users who have been invited to group
// '' = members
$q = trim($query);

if (empty($q) && (!in_array($membershiptype, array('request', 'invite', '')))) {
    $results = array('count' => 0, 'limit' => $limit, 'offset' => $offset,
        'data' => array());
}
else {
