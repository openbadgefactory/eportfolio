...
$javascript = array('jquery','js/jquery/jquery-ui/js/jquery-ui-1.8.19.custom.min.js',
    'js/customlayout.js', 'js/jquery/modernizr.custom.js',
    get_config('wwwroot') . 'local/js/lib/bootstrap.min.js', 'tablerenderer');
...
$stylesheets[] = '<link rel="stylesheet" href="' . get_config('wwwroot') . 'local/css/bootstrap.css" />';
// EKAMPUS>

// <EKAMPUS, more translations
$smarty = smarty($javascript, $stylesheets, array('view' => array('Row', 'removethisrow', 'rows', 'generatingpreview')), array('sidebars' => false));
...
function viewlayout_submit(Pieform $form, $values) {
    global $view, $SESSION, $new, $layoutrows, $layoutcolumns, $from;
    ...
    // <EKAMPUS
    redirect('/view/blocks.php?id=' . $view->get('id') . ($new ? '&new=1' : '') . (!empty($from) ? '&from=' . $from : ''));
    // EKAMPUS>
}
