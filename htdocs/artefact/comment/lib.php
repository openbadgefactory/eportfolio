...
//< EKAMPUS
function thumb_count($viewid) {
    return count_records('artefact_comment_view_user_like', 'view', $viewid, 'liked', 1);
}

function user_has_liked($view) {
    global $USER;

    $liked = false;

    // User isn't logged in, try to find the like from a cookie.
    if (!$USER->is_logged_in()) {
        $likes_encoded = get_cookie('lks');

        if (!is_null($likes_encoded)) {
            $likes = unserialize(base64_decode($likes_encoded));

            if (is_array($likes) && isset($likes[$view->get('id')])) {
                $liked = true;
            }
        }
    }
    ...
    // User is logged in.
        ...
        $thumbrecord = get_record('artefact_comment_view_user_like', 'usr',
                $USER->get('id'), 'view', $view->get('id'));

        // User hasn't liked the view yet.
        if ($thumbrecord !== false) {
            $liked = true;
        }
    }
...
function thumb_up(View $view) {
    global $USER;

    $success = false;

    if (!user_has_liked($view)) {
        $newid = insert_record('artefact_comment_view_user_like', (object) array(
            'liked' => 1,
            'usr' => $USER->is_logged_in() ? $USER->get('id') : null,
            'view' => $view->get('id')
        ), false, true);

        // Store the view into a cookie for non-loggedin users.
        if (!$USER->is_logged_in()) {
            $likes_encoded = get_cookie('lks');
            $likes = array();
            ...
            $likes[$view->get('id')] = $newid;

            set_cookie('lks', base64_encode(serialize($likes)));
        }
...
class ArtefactTypeComment extends ArtefactType {
    ...
    public function commit() {
        ...
        $data = (object)array(
            ...
            //<EKAMPUS
            'viewid'        => $this->get('viewid'), // EKAMPUS >
        );
    ...
    public static function bulk_delete($artefactids) {
        ...
        // <EKAMPUS
        // Before deleting the comments, we need to delete the related
        // annotations (annotation.parent = comment.id). Otherwise the deletion
        // fails because of foreign key constraints (and no CASCADING rules).
        // The records in artefact_parent_cache table are removed in function
        // parent::bulk_delete below, but the cache records need to be deleted
        // before deleting the annotations because of foreign key constraints.
        delete_records_select('artefact_parent_cache', "parent IN (" . $idstr . ")");
        delete_records_select('artefact', "artefacttype = 'annotate' AND parent IN (" . $idstr . ")");
        // EKAMPUS>
    ...
    public static function add_comment_form($defaultprivate=false, $moderate=false) {
        ...
        $form['elements']['submit'] = array(
            'type'  => 'submitcancel',
            'value' => array(get_string('placefeedbackbutton', 'artefact.comment'), get_string('cancel')),
        );
        return $form;
    }
...
function add_feedback_form_validate(Pieform $form, $values) {
    ...
    //< EKAMPUS
    if (empty($values['attachments']) && empty($values['message']) && empty($values['rating'])) {
    // EKAMPUS >
...
class ActivityTypeArtefactCommentFeedback extends ActivityTypePlugin {
    ...
    public function __construct($data, $cron=false) {
        ...
        if ($onartefact = $comment->get('onartefact')) { // feedback on artefact
            ...
            if (empty($this->url)) {
                // <EKAMPUS - Link to view, even if feedback on artefact.
                if (isset($data->linktoview)) {
                    $onview = (int) $data->linktoview;
                    if (!$viewrecord = get_record('view', 'id', $onview)) {
                        throw new ViewNotFoundException(get_string('viewnotfound', 'error', $onview));
                    }

                    $this->url = 'view/view.php?id=' . $onview . '&artefact=' . $onartefact;
                }
                else {
                // EKAMPUS>
