...
{* EKAMPUS >*}
  ...
  {if $ownprofile && !$showtabs}
  <div class="rbuttons userviewrbuttons">
    {* <EKAMPUS *}
    <a title="{str tag=editthisview section=view}" href="{$WWWROOT}view/blocks.php?profile=1&backto=user/view.php?id={$USERID}" class="btn">{str tag=editthisview section=view}</a>
    {* EKAMPUS >*}
    <a title="{str tag=aboutme section=artefact.internal}" href="{$WWWROOT}artefact/internal/index.php" class="btn">{str tag=aboutme section=artefact.internal}</a>
    <a title="{str tag=profileicons section=artefact.file}" href="{$WWWROOT}artefact/file/profileicons.php" class="btn">{str tag=profileicons section=artefact.file}</a>
  </div>
  {/if}
{/if}
