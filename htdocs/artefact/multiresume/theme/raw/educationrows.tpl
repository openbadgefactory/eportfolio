{if $rows}
<table id="educationrows" class="tablerenderer resumefive resumecomposite multi{$id}">
    <thead>
        <tr>
            ...
            <th>{str tag='qualification' section='artefact.resume', lang=$lang}</th>
            <th class="resumecontrols"></th>
        </tr>
    </thead>
    <tbody>
...
<a href="edit_row.php?cv={$cv}&amp;id={$id}" title="{str tag=add, lang=$lang}" class="btn">{str tag=addedurow, section=artefact.multiresume, lang=$lang}</a>
</div>
