...
$results = View::shared_to_user($query, null, $limit, $offset, $sortby,
        $sortdir, $share, $types, $ownedby, $ownerquery);
$pages = $results->data;
...
if ($pages) {
    ...
    foreach ($pages as &$page) {
        ...
        if (isset($page['is_editable']) && $page['is_editable']) {
            ...
            // copy also same
            //if it is a group copy page for itself
            if ($page['group']){
                $page['menuitems'][] = array(
            ...
            if (in_array($page['type'], array('portfolio', 'collection')) && !$page['group']){
                $page['menuitems'][] = array(
                'title' => $page['type'] == 'collection' ? get_string('copycollection', 'collection') : get_string('copyview', 'view'),
                'url' => '#',
                'classes' => 'copypage'
        ...
        $author = isset($page['user'])
                ? full_name($page['user'])
                : (isset($page['groupdata']) ? $page['groupdata']->name : '');
