...
$lock = fopen(get_config('dataroot') . '/temp/cloud.download.lock', 'w');
if (!$lock || !flock($lock, LOCK_SH)) {
    log_info("Failed to get shared lock for $path (continuing anyway)");
}
try {
    if (is_readable($path) && is_file($path)) {
        ...
        $cloud = get_field('artefact_cloudfile', 'type', 'artefact', $fileid);
        if ($cloud) {
            safe_require('artefact', 'cloudfile');
            $cf = new CloudFile($cloud);
            $cf->serve($fileid, $size, $options);
        }
        else {
            throw new NotFoundException('');
        }
