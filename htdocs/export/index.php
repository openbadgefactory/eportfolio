...
define('INTERNAL', 1);
define('MENUITEM', 'settings/export'); // <EKAMPUS
    ...
    if ($values['what'] == 'views') {
        ...
        foreach ($values as $key => $value) {
            if (substr($key, 0, 11) == 'collection_' && $value) {
                ...
                // <EKAMPUS
                // Fix wrong view order in export file (probably the fix in
                // lib.php is enough, but what the heck).
                $views = array_merge($views, get_column_sql('SELECT view FROM {collection_view} WHERE collection = ? ORDER BY displayorder ASC', array($collection)));
                // EKAMPUS>
            }
        }
    }
