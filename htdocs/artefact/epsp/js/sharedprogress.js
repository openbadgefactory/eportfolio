...
define(['jquery-loader', 'gridder', 'jquery.searchbox'], function (jq, gridder) {
    ...
    var init_observers = function () {
        ...
        _$('.sort-by input[name="sortpagesby"]').on('change', function () {
            ...
            _$('#headings').removeClass().addClass(sortby);
            _gridder.set_page(0);
            _do_search();
        });
       ...
       /* _$('.view-shared button').on('click', function () {
        ...
        _$('html').on('click', '.toggle-subfields', function (evt) {
            if (evt.target.nodeName != 'A') {
                _$(this).parents('.plan').toggleClass('open');
                check_open_plans();

                var subfields = _$(this).parents('.plan').find('.subfields');
                subfields.slideToggle('fast', function(){
                    _gridder.refresh();
                });
            }
        });
        _$('html').on('click', '.toggle-studentplans', function (evt) {
            if (evt.target.nodeName != 'A') {
                _$(this).parents('.epsp-item').toggleClass('open');
                var subfields = _$(this).parents('.epsp-item').find('.studentplans');
                subfields.slideToggle('fast', function(){
                    _gridder.refresh();
                });
            }
        });
        _$('html').on('click', '#openall a', function (evt) {
            evt.preventDefault();

            if (_$(this).text() == _opts.opentext) {
                _$('.subfields').parents('.plan, .epsp-item').addClass('open');
                _$('.subfields, .studentplans').slideDown('fast', function(){
                    _gridder.refresh();
                });
                _$(this).text( _opts.closetext).blur();
            }
            else {
                _$('.subfields').parents('.plan, .epsp-item').removeClass('open');
                _$('.subfields, .studentplans').slideUp('fast', function(){
                    _gridder.refresh();
                });
                _$(this).text( _opts.opentext).blur();
            }
            check_open_plans();
        });
    ...
    var check_open_plans = function () {
        if (_$('.plan.open').length > 0) {
            _$('#headings').addClass('open');
            _$('#headings .lastsection').css('text-indent', '-999px');
        }
        else {
            _$('#headings').removeClass('open');
            _$('#headings .lastsection').css('text-indent', '0px');
        }
    }
    ...
    var init_search_field = function () {
        if (_opts.progress) {
            //var url = 'sharedprogress.json.php?limit=' + _gridder.get_items_per_page();
            var url = 'sharedprogress.json.php?limit=500';
        }
        else {
        ...
        _$('#template-filter').searchbox({
            ...
            onkeyup: function () {
                _gridder.set_page(0);
                _$('#openall a').text( _opts.opentext).blur();
            },
            delay: 250,
    ...
    var handle_search_results = function (data) {
        _gridder.replace(data.message.html, function () {
            ...
            update_layout(data.message.total);
        });
    };
    ...
    var update_layout = function (total) {
        _gridder.get_grid().shuffle('paginate', _gridder.get_sort_opts(), {
                page:_gridder.get_page(),
                limit: 20,
        });
        _gridder.update_pagination(total);
    };
