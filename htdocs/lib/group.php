...
function group_create($data) {
    ...
    if (isset($data['shortname']) && strlen($data['shortname'])) {
        ...
        // <EKAMPUS
        $data['shortname'] = str_replace(array(' ','%'),'_',$data['shortname']);
        $data['shortname'] = str_replace(array('å','ä','Å','Ä'),'a',$data['shortname']);
        $data['shortname'] = str_replace(array('ö','Ö'),'o',$data['shortname']);
        // EKAMPUS>
        ...
        if (!preg_match('/^[a-zA-Z0-9_.()\-\/\:]{2,255}$/', $data['shortname'])) { // <EKAMPUS
        ...
        // <EKAMPUS
        if (API_GROUP && !empty($data['institution'])) {
        // EKAMPUS
    ...
    if (!isset($data['submittableto'])) {
        // <EKAMPUS
        // Only course group types are submittable.
        $data['submittableto'] = $data['grouptype'] == 'course';
        // EKAMPUS>
    }
    ...
    $data['id'] = $id;
    //< EKAMPUS dont do this for system type groups!
    if ($data['grouptype'] != 'system'){
...
function group_update($new, $create=false) {
    ...
    // <EKAMPUS
    else if (API_GROUP && !empty($new->institution) && isset($new->shortname) && strlen($new->shortname)) {
    // EKAMPUS>
    ...
    // <EKAMPUS
    if (API_GROUP && !empty($old->institution)) {
    // EKAMPUS
...
function group_prepare_usergroups_for_display($groups, $returnto='mygroups') {
    ...
    $returntoabsolute = 'interaction.pages.mygroups';
    // EKAMPUS>
    ...
    foreach ($groups as $group) {
        ...
        // <EKAMPUS
        $group->admin_id = count($group->admins) > 0 ? $group->admins[0]->id : null;
        $group->admin_name = count($group->admins) > 0 ? full_name($group->admins[0]) : null;

        $group->menuitems = array();
        if ($group->membershiptype == 'member') {
            if ($group->canleave) {
                $group->menuitems[] = array(
                    'title' => get_string('leavegroup', 'group'),
                    'url' => $wwwroot . 'group/leave.php?id=' . $group->id .
                        '&amp;returntoabsolute=' . $returntoabsolute
                );
            }
        }
        else if ($group->membershiptype == 'admin') {
            $group->menuitems[] = array(
                'title' => get_string('edit'),
                'url' => $wwwroot . 'group/edit.php?id=' . $group->id
            );
            $group->menuitems[] = array(
                'title' => get_string('delete'),
                'url' => $wwwroot . 'group/delete.php?id=' . $group->id
            );

            if ($group->requests) {
                $group->menuitems[] = array(
                    'title' => get_string('membershiprequests', 'group') . ' (' .
                        $group->requests . ')',
                    'url' => $wwwroot . 'group/members.php?id=' . $group->id .
                        '&amp;membershiptype=request'
                );
            }
        }
        else if ($group->membershiptype == 'request') {
            $group->menuitems[] = array(
                'title' => get_string('requestedtojoin', 'group')
            );
        }
        else if ($group->request) {
            $group->menuitems[] = array(
                'title' => get_string('requestjoingroup', 'group'),
                'url' => $wwwroot . 'group/requestjoin.php?id=' . $group->id .
                    '&amp;returntoabsolute=' . $returntoabsolute
            );
        }

        if($group->membershiptype && $group->membershiptype != 'invite'){
            $group->menuitems[] = array(
                'title' => get_string('edittags', 'group'),
                'url' => $wwwroot . 'group/tags.php?id=' . $group->id,
            );
            $group->tags = group_get_tags($group->id);
        }
...
function group_get_menu_tabs() {
    ...
    if ($role){
        $menu['tags'] = array(
        'path' => 'groups/tags',
        'url'  => 'group/tags.php?id='.$group->id,
    ...
    $menu['views'] = array(
        ...
        $menu['collections'] = array(
            ...
            // <EKAMPUS
            'url' => 'interaction/pages/groupcollections.php?group='.$group->id,
            // EKAMPUS>
...
//< EKAMPUS -- added systemgroups, tags, query and sort
function group_get_associated_groups($userid, $filter='all', $limit=20, $offset=0, $category='', $getsystemgroups = true, $tags = array(), $query = null, $sort = 'name') {
// EKAMPUS >
    ...
    // <EKAMPUS
    if (!$getsystemgroups) {
        $catsql .= ' AND g.grouptype != ?';
        $values[] = 'system';
    }

    if (!empty($tags)){
        $tagcount = count($tags);
        $catsql .= " AND g.id IN (SELECT `group` from group_tag WHERE ";
        $catsql .= " tag IN (" . join(',', array_map('db_quote', $tags)) . ") AND usr = $userid GROUP BY `group` HAVING COUNT(tag) = $tagcount)";
    }
    if ($query){
        $sql .= ' LEFT JOIN group_tag gt ON g.id = gt.group AND gt.usr = '.$userid;
        $like = db_ilike();
        $q = '%' . $query . '%';
        $catsql .= " AND (g.name $like ? OR g.description $like ? OR gt.tag $like ?)";
        $values[] = $q;
        $values[] = $q;
        $values[] = $q;
    }
    ...
    $sql = '
        SELECT g1.id, g1.name, g1.description, g1.public, g1.jointype, g1.request, g1.grouptype, g1.submittableto,
            ...
            COUNT(gmr.member) AS requests, g1.editwindowstart, g1.editwindowend'. /* <EKAMPUS */', g1.mtime, g1.ctime, g1.institution AS inst ' . /* EKAMPUS> */'
        FROM (
            SELECT g.id, g.name, g.description, g.public, g.jointype, g.request, g.grouptype, g.submittableto,
                ...
                COUNT(gm.member) AS membercount, g.editwindowstart, g.editwindowend'. /* <EKAMPUS */', g.mtime, g.ctime, g.institution ' . /* EKAMPUS> */'
            FROM {group} g
...
// <EKAMPUS
function group_get_user_groups($userid=null, $roles=null, $getsystemgroups = true) {
// EKAMPUS>
    global $USER;
    ...
    // <EKAMPUS
    if (!isset($usergroups[$userid]) || !$getsystemgroups) {
        $wheresql = '';
        $params = array($loggedinid, $userid);

        if (!$getsystemgroups) {
            $wheresql = 'AND g.grouptype != ?';
            $params[] = 'system';
        }
        // EKAMPUS>
...
function group_get_homepage_view($groupid) {
    ...
    // <EKAMPUS
    return new View($v ? $v->id : -1, (array)$v);
    // EKAMPUS>
}
...
/*< EKAMPUS */
function group_update_tags($groupid, $values){
    global $USER, $SESSION;
    ...
    if (!$group = get_record('group', 'id', $groupid, 'deleted', 0)) {
        throw new NotFoundException("group_update_tags: group not found: $groupid");
    }

    if (!is_array($values['tags'])) {
        throw new SystemException("group_update_tags: values must be an array");
    }

    $oldtags = group_get_tags($groupid);

    db_begin();
    $dataobject = array('group' => $groupid, 'usr' => $USER->id);

    foreach ($values['tags'] as $tag){
        $dataobject['tag'] = $tag;
        ensure_record_exists('group_tag', (object)$dataobject, (object)$dataobject);
    }
    //delete tags which are not in the new array
    foreach ($oldtags as $oldtag){
        if (!in_array($oldtag, $values['tags'])){
            delete_records('group_tag', 'group', $groupid, 'usr', $USER->id, 'tag', $oldtag);
        }
    }

    db_commit();
}
function group_get_tags($groupid){
    global $USER;
    if (!$group = get_record('group', 'id', $groupid, 'deleted', 0)) {
        throw new NotFoundException("group_get_tags: group not found: $groupid");
    }

    if ($result = get_records_sql_array("SELECT tag FROM {group_tag} WHERE `group` = ? AND usr = ?", array($groupid, $USER->id))) {
...
function get_group_tags($userid) {

        $tagrecords = get_records_sql_array(
                "SELECT DISTINCT(tag) AS tag
                FROM {group_tag} WHERE usr = ? AND `group` IN (
                SELECT `group` FROM {group_member} WHERE member = ? )", array($userid, $userid));
        $tags = array();
